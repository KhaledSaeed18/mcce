...
src/
  ...
  components/
    ...
    <domain>/     Shared presentational components (about, contact, course, curriculum, drive,
                  exams, faq, footer, gpa, legal, marketing, nav, providers, recent,
                  resources, seo, sitemap). JSX only, no data fetching.
  ...
  lib/            Pure utilities and server-side data access (TanStack Start server functions).
                  No JSX. Grouped by domain (contact, curriculum, drive, gpa, resources, seo).
  config/         Named constants and static config, grouped by domain. The resource hub
                  catalog is authored here, under config/resources/catalog and repos.
  data/           Generated data artifacts (the Drive index, the resources index).
                  Machine-written, never hand-edited.
  assets/         Static brand assets.
scripts/
  sync-drive/     Crawls Google Drive and regenerates the Drive index, sitemap, and feed.
  build-resources/ Validates the resource catalog, fetches brand icons, checks links, and
                  writes src/data/resources-index.json.
...
| `pnpm build:resources` | Validate the resource catalog and regenerate its index and icons. Pass `--skip-links` or `--skip-icons` to stay offline, `--check` to compare with the committed index |
...
- **`src/data/resources-index.json`** and **`public/resources/icons/`** are generated by
  `pnpm build:resources` and excluded the same way. Edit the catalog under
  `src/config/resources/` instead, then rebuild and commit the output together.
...
The resource hub (`/resources`) is a hand-curated catalog. To add a tool:

1. Pick the category file under `src/config/resources/catalog/` and add one entry with `id`
   (kebab-case, never changed later), `name`, a one-line `description` under 140 characters,
...
4. Add `thesisStages` when the tool belongs on the thesis page, and `privacyNote` for any cloud
   tool that takes uploads.
5. For a GitHub repository, add a row to the matching file under `src/config/resources/repos/`
   with its licence, reuse class, and maintenance state, and `resourceId` when a tool entry
   exists for it.
6. Run `pnpm build:resources` and commit the catalog change together with the regenerated
   index and any new icon files.

Removing a resource is a deletion with the reason in the commit body ("free tier removed",
"project archived"). Nothing is kept as deprecated.
